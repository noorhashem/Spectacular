...
#import "CurrencyDetectionView.h"
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [showImage setImage:chosenImage];
    [showImage setHidden:false];
    input_image = chosenImage;
...
- (void) viewdidloadMethod {
     ...
     [self sayText : @"Tap to use currency detection or swipe back to the right"];
      imagePicker.delegate = self;
}


- (void) sayText:(NSString *)strtext {
...
- (void)setPredictionValues:(std::vector<std::pair<float, std::string>>)newValues {
    ...
    for  (const auto& item : newValues) {
        ...
        const float labelOriginX = leftMargin;
        //      (leftMargin + valueWidth + labelMarginX);
        
        NSString *nsLabel = [NSString stringWithCString:label.c_str()
                                               encoding:[NSString defaultCStringEncoding]];
        
        [self sayText:nsLabel];
